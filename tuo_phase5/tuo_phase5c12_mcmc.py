...
from tuo_phase5c11a_joint_likelihood import (
...
_PARAM_COLS = ['beta0', 'nu2', 'x_ini', 'h', 'omega_b', 'omega_c']
_UCOLS = ['u_beta', 'u_nu', 'u_x', 'u_h', 'u_omega_b', 'u_omega_c']
...
def _log_sigmoid_jac(u: float) -> float:
    # log(s*(1-s)) computed stably
    s = _sigmoid(u)
    return math.log(max(s * (1.0 - s), 1e-300))


def u_to_theta(u: np.ndarray, cfg: Dict[str, Any]) -> Tuple[np.ndarray, float]:
    """Map unconstrained variables u to physical theta and return log|Jac|."""
    u_beta, u_nu, u_x, u_h, u_ob, u_oc = map(float, u)
    pri = cfg['priors']
    sec = cfg['sector']

    beta_floor = float(cfg['beta_floor'])
    beta_max = float(cfg['beta_max'])
    nu_min = float(cfg['nu_min'])
    nu_max = float(cfg['nu_max'])
    x_min = float(cfg['x_min'])
    x_max = float(cfg['x_max'])

    # finite-range logistic maps
    width_beta = beta_max - beta_floor
    s_beta = _sigmoid(u_beta)
    beta_abs = beta_floor + width_beta * s_beta
    if sec == 'positive':
        beta0 = beta_abs
    elif sec == 'negative':
        beta0 = -beta_abs
    else:
        # fallback free sector: bounded logistic in [-beta_max, beta_max]
        s2 = _sigmoid(u_beta)
        beta0 = -beta_max + 2.0 * beta_max * s2
        width_beta = 2.0 * beta_max
        beta_floor = -beta_max
        # log jac with centered interval
        logJ_beta = math.log(max(width_beta, 1e-300)) + _log_sigmoid_jac(u_beta)
        # other transforms below
        pass

    width_nu = nu_max - nu_min
    s_nu = _sigmoid(u_nu)
    nu2 = nu_min + width_nu * s_nu

    width_x = x_max - x_min
    s_x = _sigmoid(u_x)
    x_ini = x_min + width_x * s_x

    # whitened gaussians (linear map; jacobian constant can be dropped)
    h = float(pri['h_mean']) + float(pri['h_sigma']) * u_h
    omega_b = float(pri['omega_b_mean']) + float(pri['omega_b_sigma']) * u_ob
    omega_c = float(pri['omega_c_mean']) + float(pri['omega_c_sigma']) * u_oc

    if sec in ('positive', 'negative'):
        logJ_beta = math.log(max(width_beta, 1e-300)) + _log_sigmoid_jac(u_beta)
    logJ_nu = math.log(max(width_nu, 1e-300)) + _log_sigmoid_jac(u_nu)
    logJ_x = math.log(max(width_x, 1e-300)) + _log_sigmoid_jac(u_x)
    logJ = logJ_beta + logJ_nu + logJ_x

    theta = np.array([beta0, nu2, x_ini, h, omega_b, omega_c], dtype=float)
    return theta, float(logJ)


def theta_to_u(theta: np.ndarray, cfg: Dict[str, Any]) -> np.ndarray:
    beta0, nu2, x_ini, h, omega_b, omega_c = map(float, theta)
    pri = cfg['priors']
    sec = cfg['sector']

    def inv_logit_interval(x, lo, hi):
        eps = 1e-12
        y = (x - lo) / (hi - lo)
    ...
    beta_max = float(cfg['beta_max'])
    if sec == 'positive':
        beta_abs = max(beta_floor, min(beta_max, beta0))
        u_beta = inv_logit_interval(beta_abs, beta_floor, beta_max)
    elif sec == 'negative':
        beta_abs = max(beta_floor, min(beta_max, abs(beta0)))
        u_beta = inv_logit_interval(beta_abs, beta_floor, beta_max)
    else:
        u_beta = inv_logit_interval(beta0, -beta_max, beta_max)

    u_nu = inv_logit_interval(nu2, float(cfg['nu_min']), float(cfg['nu_max']))
    u_x = inv_logit_interval(x_ini, float(cfg['x_min']), float(cfg['x_max']))
    u_h = (h - float(pri['h_mean'])) / float(pri['h_sigma'])
    u_ob = (omega_b - float(pri['omega_b_mean'])) / float(pri['omega_b_sigma'])
    u_oc = (omega_c - float(pri['omega_c_mean'])) / float(pri['omega_c_sigma'])
    return np.array([u_beta, u_nu, u_x, u_h, u_ob, u_oc], dtype=float)


def _physical_bounds_ok(theta: np.ndarray, cfg: Dict[str, Any]) -> bool:
    beta0, nu2, x_ini, h, omega_b, omega_c = map(float, theta)
    sec = cfg['sector']
    beta_floor = float(cfg['beta_floor'])
    beta_max = float(cfg['beta_max'])
    if sec == 'positive' and not (beta_floor <= beta0 <= beta_max):
        return False
    if sec == 'negative' and not (-beta_max <= beta0 <= -beta_floor):
        return False
    if sec == 'free' and not (-beta_max <= beta0 <= beta_max):
        return False
    if not (float(cfg['nu_min']) <= nu2 <= float(cfg['nu_max'])):
        return False
    if not (float(cfg['x_min']) <= x_ini <= float(cfg['x_max'])):
        return False
    if not (0.55 <= h <= 0.85):
        return False
...
def init_worker(worker_cfg: Dict[str, Any]):
    ...
    sector_cfg = dict(worker_cfg['sector_cfg'])
...
def worker_logpost_u(u: np.ndarray) -> Tuple[float, np.ndarray, np.ndarray]:
    global _WORKER
    cfg = _WORKER['sector_cfg']
    theta, logJ = u_to_theta(u, {'priors': _WORKER['priors'], **cfg})
    if not _physical_bounds_ok(theta, {'priors': _WORKER['priors'], **cfg}):
        return -np.inf, np.array([np.inf] + [np.nan] * (len(_BLOB_COLS) - 1), dtype=float), theta
    ...
    # add Jacobian for transformed sampling coordinates
    logpost = -0.5 * (float(res['chi2_total']) + float(chi2_prior)) + logJ
    ...
    return float(logpost), blob, theta


def _run_chain_serial(args_tuple) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float]:
    rng_seed, init_u, nsteps, burn, adapt_every, proposal_cov = args_tuple
    rng = np.random.default_rng(int(rng_seed))
    d = init_u.size
    usamples = np.empty((nsteps, d), dtype=float)
    thetas = np.empty((nsteps, d), dtype=float)
    logps = np.empty(nsteps, dtype=float)
    blobs = np.empty((nsteps, len(_BLOB_COLS)), dtype=float)

    u = np.array(init_u, dtype=float)
    lp, blob, theta = worker_logpost_u(u)
    ...
    for i in range(nsteps):
        prop = rng.multivariate_normal(u, proposal)
        lp_prop, blob_prop, theta_prop = worker_logpost_u(prop)
        ...
        logps[i] = lp
        blobs[i] = blob
        chain_hist.append(u.copy())
        if adapt_every > 0 and (i + 1) % adapt_every == 0 and (i + 1) > max(20, burn // 2):
            arr = np.array(chain_hist[max(0, len(chain_hist) - 400):], dtype=float)
            if arr.shape[0] > d + 5:
                cov = np.cov(arr.T) + 1e-6 * np.eye(d)
                proposal = cov * ((2.38 ** 2) / d)
    runtime = time.time() - t0
    return usamples, thetas, logps, blobs, accepted / float(nsteps), runtime
...
def _load_map_json(path: Optional[Path]) -> Optional[Dict[str, float]]:
    if path is None:
        return None
    with open(path, 'r', encoding='utf-8') as f:
        obj = json.load(f)
    if 'best_fit_tuo' in obj:
        return obj['best_fit_tuo']
    return obj


def main():
    ap = argparse.ArgumentParser(description='TUO Fase 5C.1.2 MCMC (reparametrized + beta sectors)')
    ...
    ap.add_argument('--outdir', type=Path, default=Path('tuo_phase5c12_run'))
    ...
    ap.add_argument('--sector', choices=['positive', 'negative', 'free'], default='positive')
    ap.add_argument('--beta-floor', type=float, default=1e-4)
    ap.add_argument('--beta-max', type=float, default=0.10)
    ap.add_argument('--nu-min', type=float, default=0.005)
    ap.add_argument('--nu-max', type=float, default=0.06)
    ap.add_argument('--x-min', type=float, default=1e-4)
    ap.add_argument('--x-max', type=float, default=0.60)
    ap.add_argument('--map-json', type=Path, default=None)
    # physical init (used if no map-json)
    ap.add_argument('--beta0-init', dest='beta0_init', type=float, default=0.02)
    ap.add_argument('--nu2-init', dest='nu2_init', type=float, default=0.025)
    ap.add_argument('--x-ini-init', dest='x_ini_init', type=float, default=0.05)
    ap.add_argument('--h-init', type=float, default=0.680)
    ap.add_argument('--omega-b-init', dest='omega_b_init', type=float, default=0.02233)
    ap.add_argument('--omega-c-init', dest='omega_c_init', type=float, default=0.1199)
    # proposal scales in transformed space
    ap.add_argument('--sig-u-beta', dest='sig_u_beta', type=float, default=0.25)
    ap.add_argument('--sig-u-nu', dest='sig_u_nu', type=float, default=0.20)
    ap.add_argument('--sig-u-x', dest='sig_u_x', type=float, default=0.25)
    ap.add_argument('--sig-u-h', dest='sig_u_h', type=float, default=0.30)
    ap.add_argument('--sig-u-omega-b', dest='sig_u_omega_b', type=float, default=0.30)
    ap.add_argument('--sig-u-omega-c', dest='sig_u_omega_c', type=float, default=0.30)
    # priors
    ...
    map_best = _load_map_json(args.map_json)
    if map_best is not None:
        beta0_init = float(map_best.get('beta0', args.beta0_init))
        nu2_init = float(map_best.get('nu2', args.nu2_init))
        x_ini_init = float(map_best.get('x_ini', args.x_ini_init))
        h_init = float(map_best.get('h', args.h_init))
        omega_b_init = float(map_best.get('omega_b', args.omega_b_init))
        omega_c_init = float(map_best.get('omega_c', args.omega_c_init))
    else:
        beta0_init = float(args.beta0_init)
        nu2_init = float(args.nu2_init)
        x_ini_init = float(args.x_ini_init)
        h_init = float(args.h_init)
        omega_b_init = float(args.omega_b_init)
        omega_c_init = float(args.omega_c_init)

    # sector-specific default sign repair
    if args.sector == 'positive' and beta0_init <= 0.0:
        beta0_init = max(args.beta_floor * 10.0, 0.01)
    if args.sector == 'negative' and beta0_init >= 0.0:
        beta0_init = -max(args.beta_floor * 10.0, 0.01)
    ...
    sector_cfg = {
        'sector': args.sector,
        'beta_floor': float(args.beta_floor), 'beta_max': float(args.beta_max),
        'nu_min': float(args.nu_min), 'nu_max': float(args.nu_max),
    ...
    theta_init = np.array([beta0_init, nu2_init, x_ini_init, h_init, omega_b_init, omega_c_init], dtype=float)
    init_u = theta_to_u(theta_init, {'priors': priors, **sector_cfg})
    d = init_u.size
    proposal_diag = np.array([
        args.sig_u_beta, args.sig_u_nu, args.sig_u_x,
        args.sig_u_h, args.sig_u_omega_b, args.sig_u_omega_c,
    ], dtype=float)
    ...
    for _ in range(args.nchains):
        u0 = init_u + rng.normal(scale=proposal_diag, size=d)
        lp, _, _ = worker_logpost_u(u0)
        tries = 0
        local_scale = proposal_diag.copy()
        while not np.isfinite(lp) and tries < 200:
            if tries in (50, 100, 150):
                local_scale *= 0.5
            u0 = init_u + rng.normal(scale=local_scale, size=d)
            lp, _, _ = worker_logpost_u(u0)
            tries += 1
        if not np.isfinite(lp):
            raise RuntimeError('Could not initialize finite posterior for one chain in transformed space')
        chain_inits.append(u0)
    ...
    u_list, theta_list, logps_list, blobs_list, accs, runtimes = zip(*results)
    nch = len(theta_list)
    chains_theta = np.stack(theta_list, axis=0)
    chains_u = np.stack(u_list, axis=0)
    logps = np.stack(logps_list, axis=0)
    blobs = np.stack(blobs_list, axis=0)

    for j in range(nch):
        pd.DataFrame(theta_list[j], columns=_PARAM_COLS).to_csv(outdir / f'chain_{j}.csv', index=False)
        pd.DataFrame(u_list[j], columns=_UCOLS).to_csv(outdir / f'chain_{j}_u.csv', index=False)
        np.save(outdir / f'chain_{j}_samples.npy', theta_list[j])
        np.save(outdir / f'chain_{j}_u.npy', u_list[j])
    ...
    flat_theta = chains_theta[:, keep, :].reshape(-1, d)
    flat_u = chains_u[:, keep, :].reshape(-1, d)
    flat_logps = logps[:, keep].reshape(-1)
    flat_blobs = blobs[:, keep, :].reshape(-1, blobs.shape[-1])

    cols = _PARAM_COLS + _UCOLS + ['logpost'] + _BLOB_COLS
    flat_df = pd.DataFrame(np.column_stack([flat_theta, flat_u, flat_logps, flat_blobs]), columns=cols)
    flat_df.to_csv(outdir / 'flat_samples_with_blobs.csv', index=False)

    best_idx = int(np.argmin(flat_blobs[:, 0]))
    best_theta = flat_theta[best_idx]
    best_blob = flat_blobs[best_idx]
    Rhat_theta = gelman_rubin(chains_theta[:, keep, :])
    Rhat_u = gelman_rubin(chains_u[:, keep, :])

    delta_k = 3  # extra TUO params wrt LCDM background+priors sector
    dchi2 = float(best_blob[0] - lcdm['chi2_total'])
    dchi2eff = float((best_blob[0] + best_blob[4]) - lcdm['chi2_posterior_effective'])
    daic = dchi2 + 2.0 * delta_k
    daiceff = dchi2eff + 2.0 * delta_k
    ...
    summary = {
        ...
        'Rhat_theta': {
            'beta0': float(Rhat_theta[0]), 'nu2': float(Rhat_theta[1]), 'x_ini': float(Rhat_theta[2]),
            'h': float(Rhat_theta[3]), 'omega_b': float(Rhat_theta[4]), 'omega_c': float(Rhat_theta[5]),
        },
        'Rhat_u': {
            'u_beta': float(Rhat_u[0]), 'u_nu': float(Rhat_u[1]), 'u_x': float(Rhat_u[2]),
            'u_h': float(Rhat_u[3]), 'u_omega_b': float(Rhat_u[4]), 'u_omega_c': float(Rhat_u[5]),
        },
        'best_fit_tuo': {
        ...
        'delta_chi2_tuo_minus_lcdm': dchi2,
        'delta_chi2eff_tuo_minus_lcdm': dchi2eff,
        'delta_AIC_tuo_minus_lcdm': daic,
        'delta_AICeff_tuo_minus_lcdm': daiceff,
        'posterior_quantiles_16_50_84': {
            'beta0': [float(x) for x in np.quantile(flat_theta[:, 0], [0.16, 0.50, 0.84])],
            'nu2': [float(x) for x in np.quantile(flat_theta[:, 1], [0.16, 0.50, 0.84])],
            'x_ini': [float(x) for x in np.quantile(flat_theta[:, 2], [0.16, 0.50, 0.84])],
            'h': [float(x) for x in np.quantile(flat_theta[:, 3], [0.16, 0.50, 0.84])],
            'omega_b': [float(x) for x in np.quantile(flat_theta[:, 4], [0.16, 0.50, 0.84])],
            'omega_c': [float(x) for x in np.quantile(flat_theta[:, 5], [0.16, 0.50, 0.84])],
        },
    }
